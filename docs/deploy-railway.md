...
Ничего делать не нужно: сервер применяет их сам при старте и пишет в лог
`схема БД актуальна`. Drizzle помнит выполненные миграции в таблице
`__drizzle_migrations`, поэтому повторные запуски безвредны.

Если схему применить не удалось, сервер не поднимется вовсе — это лучше,
чем стартовать и упасть позже, на первом входе игрока.
...
| Прогресс не сохраняется | не проставлен `DATABASE_URL` — в логе будет `db=off` |
| `не удалось применить миграции` при старте | `DATABASE_URL` указывает не на ту базу или нет прав на создание таблиц |
